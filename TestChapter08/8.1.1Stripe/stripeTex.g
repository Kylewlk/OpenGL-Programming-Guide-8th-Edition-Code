...
void main(void)
{
	...
	float frac1 = clamp(scaledT/Fuzz, 0.0, 1.0);//出去fuzz区域，其他都时1.
	float frac2 = clamp((scaledT-Width)/Fuzz, 0.0, 1.0);//出了fuzz+width区域，其他都是1
	frac1 = frac1 * (1-frac2);//计算是否时条带颜色，除了放走样区域，其他要么为0，要么为1
	frac1 = frac1 * frac1 *(3.0- (2.0 * frac1));//Hermite插值反走样运算，实际上2、3行代码已经具备反走样能力，只是加上这句话反走样效果更好。
